...
何をするにも、他の関数型言語のユニット型に相応する**終対象**が必ず必要なため、まずはこれを定義します。

圏論において、**終対象** (terminal object) とは「どの対象からも、ちょうど一つだけ射が存在する対象」のことです。これは「終点」のような役割を果たし、情報を持たない（あるいは唯一の値しか持たない）対象を表します。
...
ここで `*a` は型変数を表しており、 `!` は単なる射ではなく自然変換（一般的な関数型言語における多相関数に相当）になっています。
...
- **プログラミングとの対応**: 余帰納的データ型（coinductive types）、遅延評価
...
- **プログラミングとの対応**: 帰納的データ型（inductive types）、パターンマッチング
...
圏論において、直積 `a × b` は「二つの対象 `a` と `b` の情報を両方とも保持する対象」として定義されます。これは以下の性質を持ちます:
...
指数対象 `bᵃ` (CPLでは `exp(a,b)` と表記) は「`a` から `b` への射全体を表す対象」です。これは以下の性質を持ちます:

- 評価射 `eval: exp(a,b) × a → b` が存在し、関数を値に適用できる
- 任意の射 `f: c × a → b` に対して、カリー化された射 `curry(f): c → exp(a,b)` が一意に存在する
...
- 任意の対象 `a` と射 `z: 1 → a` および `f: a → a` に対して、帰納的に定義された唯一の射 `pr(z,f): ℕ → a` が存在する
...
- 任意の対象 `x` への射 `f: a → x` と `g: b → x` があれば、それらをケース分けで組み合わせた唯一の射 `case(f,g): a + b → x` が存在する

これは直積の「矢印を逆にした」双対概念であり、圏論における対称性の良い例です。

図式で描くと以下のようになります（直積の図と比較して、矢印が逆になっていることを確認してください）：
...
- `fold(h,t): x → inflist(a)` により、無限リストを展開（unfold）
